...
d <- read.csv("./gazetest.csv", header = TRUE, fileEncoding = "utf8")
...
for (m in c(3, 4, 5)) {
    ...
    for (n in 6:7) {
        ...
        d %>%
            ...
            ylab("")+
            ggtitle(title)
        ggsave(sprintf("graph/gaze/%s/boxplot_%s.png",name[[m]],title))
        ...
        # ggsave(sprintf("graph/gaze/%s/bar_mean_%s.png",name[[m]],title))
        ...
        capture.output(out, file = sprintf("gaze/anova/%s/art_anova_%s.txt",name[m],title))

        capture.output(anovakun(spreaddata, "sA", 3, holm = T, mau = T, eta = T), file = sprintf("gaze/anova/%s/anova_%s.txt",name[m],title))
