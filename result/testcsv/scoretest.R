...
d <- read.csv("./scoretest.csv", header = TRUE, fileEncoding = "utf8")
...
d <- mutate(d, 挙手時間 = lefthandtime+righthandtime)
d <- mutate(d, 挙手回数 = lefthand+righthand)
...
for (m in c(3, 4, 5)) {
    ...
    for (n in 6:16) {
        ...
        d %>%
            ...
            ylab("")+
            ggtitle(title)
        ggsave(sprintf("graph/score/%s/boxplot_%s.png",name[[m]],title))
        ...
        # ggsave(sprintf("graph/score/%s/bar_mean_%s.png",name[[m]],title))
        ...
        capture.output(anovakun(spreaddata, "sA", 3, holm = T, mau = T, eta = T), file = sprintf("score/anova/%s/anova_%s.txt",name[m],title))


        capture.output(out, file = sprintf("score/anova/%s/art_anova_%s.txt",name[m],title))
    }
}